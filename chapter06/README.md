...
- continuations to be enqueued are denoted as `cont. of x`
...
- Onne OS thread is spawned on each processor, `T1` for processor one, and `T2` for processor two
...
- Blocking handling
  ...
  - once the hosting goroutines is unblocked, the host OS thread attempts to steal back a context from one of the other OS threads so that it can continue executing the previously blocked goroutine
